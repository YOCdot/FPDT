...
from models import build_model as build_tfpd

from util.scheduler import create_scheduler


def get_args_parser():
    parser = argparse.ArgumentParser('Set TFPD', add_help=False)
    ...
    parser.add_argument('--img_size', default=672, type=int)
    ...
    parser.add_argument('--model_name', default='tiny', type=str,
                        help="Name of the whole model")
    parser.add_argument('--pre_trained', default='',
                        help="vit pre-train file")
    parser.add_argument('--neck_pt', default='',
                        help='pre-train weight of neck')
    parser.add_argument('--encoder_pt', default='',
                        help='pre-train weight of encoder')
...
def main(args):
    ...
    model, criterion, postprocessors = build_tfpd(args)
    ...
    if args.resume:
        ...
        if not args.eval and 'optimizer' in checkpoint and 'lr_scheduler' in checkpoint and 'epoch' in checkpoint:
            ...
            lr_scheduler.load_state_dict(checkpoint['lr_scheduler'])  # TODO: lr_scheduler
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser('ConvTention Detector train & eval script', parents=[get_args_parser()])
    ...
    if args.model_name == "tiny":
        ...
        raise ValueError(f"{args.model_name} does not exists!")
