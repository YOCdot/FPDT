...
def build_yolos(model_name="tiny"):
    ...
    class Params:
        def __init__(self, backbone_name):
            ...
            if self.backbone_name == "tiny":
                self.init_pe_size = [800, 1333]
                self.mid_pe_size = None
            elif self.backbone_name == "small":
                self.init_pe_size = [512, 864]
                self.mid_pe_size = [512, 864]
            else:
                raise ValueError("no such T-SSD!")

    args = Params(backbone_name=model_name)
    model = build(args=args)[0]
...
if __name__ == "__main__":
    ...
    yolos = build_yolos(model_name="tiny")
    sample = torch.ones(size=(8, 3, 672, 672), dtype=torch.float32)
    out = yolos(sample)
    macs, params = thop.profile(yolos, inputs=(sample,))
